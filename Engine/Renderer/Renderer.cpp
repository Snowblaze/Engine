...
void Renderer::CreateRenderPass()
{
	...
	{
		throw std::runtime_error("Failer to create render pass!");
	}
}
...
void Renderer::CreateImage(uint32_t width, uint32_t height, uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory)
{
	...
	{
		throw std::runtime_error("Failed to crete image!");
	}
...
void Renderer::CreateVertexBuffer()
{
	...
	CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_SHARING_MODE_CONCURRENT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertexBuffer, vertexBufferMemory);
...
void Renderer::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkSharingMode sharingMode, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory)
{
...
VkCommandBuffer Renderer::BeginSingleTimeTransferCommands()
{
...
void Renderer::EndSingleTimeTransferCommands(VkCommandBuffer commandBuffer)
{
...
void Renderer::CreateIndexBuffer()
{
	...
	CreateBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_SHARING_MODE_CONCURRENT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, indexBuffer, indexBufferMemory);
...
void Renderer::WindowPosCallback(GLFWwindow* window, int width, int height)
{
	auto app = reinterpret_cast<Renderer*>(glfwGetWindowUserPointer(window));
	//app->recreateSwapChain();
	app->DrawFrame();
